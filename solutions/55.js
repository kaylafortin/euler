...
import { buildNumberFromDigits, getDigitsOfNumber, isPalindrome } from '../helpers/utils.js';
...
const getReverseSum = (num) => {
    const reverseDigits = getDigitsOfNumber(num).reverse();
    const reverseNumber = buildNumberFromDigits(reverseDigits);
...
const solution = ({ max }) => {
    ...
    while (num < max) {
        let iteration = 0;
        let isNumPalindrome = false;
        let currentSum = num;
        while (!isNumPalindrome && iteration < LIMIT) {
            currentSum = getReverseSum(currentSum);
            isNumPalindrome = isPalindrome(currentSum);
            iteration++
        }
        if (!isNumPalindrome) count++
        num++
    }
    return count;
}
