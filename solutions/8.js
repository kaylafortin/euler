import { template } from '../helpers/template.js';
import { getArrayProduct } from '../helpers/utils.js';

/**
 * The four adjacent digits in the 1000-digit number that have the greatest product are 9 × 9 × 8 × 9 = 5832.

 Find the thirteen adjacent digits in the 1000-digit number that have the greatest product. What is the value of this product?
 */

const NUMBER = '';

const TEST_ANSWER = 5832;
const TEST_ARGS = {
    digits: 4,
}
const ARGS = { digits: 13 }

const convertNumberToString = (num) => `${num}`;
const splitString = (str) => str.split('');

const getSubstring = (str, start, end) => str.substring(start, end);

const getProductOfString = (start, end) => {
    const subStr = getSubstring(NUMBER, start, end);
    const stringArray = splitString(subStr);
    return getArrayProduct(stringArray);
}

const getEnd = (position, digits) => position + digits

const solution = ({ digits }) => {
    const length = NUMBER.length;
    const maxLength = length - digits;
    let sum = 0;
    for (let i = 0; i < maxLength; i++) {
        const end = getEnd(i, digits);
        const substringProduct = getProductOfString(i, end)
        if (substringProduct > sum) {
            sum = substringProduct;
        }
    }
    return sum
}

template(ARGS, TEST_ARGS, TEST_ANSWER, solution)